...
Добавлена модель Seller, связанная с моделью Book. У одного продавца может быть много книг (связь один-ко-многим).
По контракту у продавца есть обязательные поля:
- id
- first_name
- last_name
- e_mail
- password
- а так же дополнительно: total_books (каунтер книг)

Пароль продавца не выводится в ответах эндпойнтов. Есть валидация поля e_mail.\
При удалении продавца удаляются так же все связанные с ним книги.
